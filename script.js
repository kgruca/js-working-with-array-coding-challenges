...
const checkDogs = function(dogsJulia, dogsKate) {
  ...
  const dogsJAndK = dogsJuliaCopy.concat(dogsKate);

  dogsJAndK.forEach(function(age, i) {
...
const calcAverageHumanAge = function(ages) {
  const humanAge = ages.map(age => age <= 2 ? age * 2 : age = 16 + 4 * age);
  const adults = humanAge.filter(age => age > 18);
  // const avg = adults.reduce((acc, age) => acc += age, 0) / adults.length;
  // can do above like this:
...
// 1.
dogs.forEach(dog => dog.recommendedFood = Math.floor((dog.weight 
  ** 0.75 * 28)));
